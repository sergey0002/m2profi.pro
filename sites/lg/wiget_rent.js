...
$('#h_adress').on('change', function() {
    ...
    sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://em.m2profi.pro.test/sahmatka/ajax_router.php?ctr=rentobjects&act=display',0); // Грузим содержимое селек
});
...
$( "#rentsearch input,#rentsearch select" ).change(function() {
	   $('#rent_home_id').attr('value',''); // Очистка дома
       sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://em.m2profi.pro.test/sahmatka/ajax_router.php?ctr=rentobjects&act=display',0,'progressbar','',function postload()
 {
	 /*
	 $(document).snowfall({
            flakeCount: 1,
