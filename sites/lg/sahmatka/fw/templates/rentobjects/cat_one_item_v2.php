...
///print '<pre>';
 ...
 <div class="row card_long" data-id="<?=$v['rent_home_id']?>">
 		...
 		foreach($files as $file) 
		{
			...
			{		
				...
				if('jpg'==end(explode(".", $src)))
				{
					...
					?><div class="swiper-slide"> <img src="https://{$GLOBALS['config']['domain']}/thumbs/?name=/sahmatka/<?=$src;?>&w=800&h=800" alt="" style="max-height:350px; width: auto;"></div> <? 
				}
				else
				{
					?><div class="swiper-slide"> <img src="https://{$GLOBALS['config']['domain']}/<?=$src;?>" alt="" style="max-height:350px; width: auto;"></div> <? 
	...
	<div class="col-lg-3">
		<a data-wrapper="#ajax-wrapper" class="ajfb" data-url="https://{$GLOBALS['config']['domain']}/sahmatka/ajax_router.php?ctr=rentobjects&act=cardn&id=<?=$v['rent_objects_id']?>"  >
