...
function act__order1()
{
    ...
    // Получаем объект квартиры
    $apartment = $mysql->get_arr(
        'SELECT * FROM `apartaments` WHERE home_id="'.$home_id.'" AND apartment_num="'.$apartment_num.'"', 1
    );
    if (!$apartment) {
    ...
    // Получаем актуальную бронь (последнюю по дате)
    $broni = $mysql->get_arr(
        'SELECT * FROM broni 
         LEFT JOIN users ON broni.user_id = users.id
         WHERE home_id="'.$home_id.'" 
           AND section_id="'.$apartment['section_id'].'"
           AND floor="'.$apartment['floor'].'"
           AND apartments_num="'.$apartment_num.'"  
         ORDER BY date DESC', 1
    );
    $stat = isset($apartment['status2']) ? $apartment['status2'] : '';
    ...
    if ($subact && $broni && $broni['broni_id']) {
        if ($subact == 'upbroni') {
            // Продление брони
            $sa->up_broni($broni['broni_id'], 4, 'Продление пользователем');
            $date_start = time();
            ...
            // Отмена брони
            $sa->up_broni($broni['broni_id'], 2, 'Отмена пользователем');
            $done_message = "Бронь отменена. Квартира снова доступна для бронирования.";
        } else {
            $done_message = "Неизвестное действие!";
        }

        // После любого subact сразу выводим done-шаблон и return
        $data = [
            'apartment'     => $apartment,
            'broni'         => $broni,
            'stat'          => $stat,
        ...
        $this->tpl($data, 'apartments', 'form_broni_done');
        return;
    }
    ...
    if (
        $broni
        && in_array($stat, ['4', '5'])
        && $broni['user_id'] == $_SESSION['sh_id']
        && $is_user
    ) {
        $date_start = strtotime($broni['date']);
        $date_end = $date_start + 14*24*3600;
        $days_left = ceil(($date_end - time()) / (24*3600));
        if ($days_left > 0) {
            $done_message = "У вас уже есть действующая бронь на эту квартиру.<br>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$show_done_template) {
        ...
        if (in_array($_SESSION['sh_login'], ['admin','em_nsv','demo_admin'])) {
            ...
            // Проверка: есть ли в базе broni бронь с таким ID
            $valid_bron = 0;
            ...
            $stat = isset($broni['status']) ? $broni['status'] : '';
        ...
        // 2. Пользователь-агент: только если квартира свободна
            ...
            if ($stat != '' && $stat != '2' && $stat != '5' && $stat != '0') {
                ...
                if (!$err_m) {
                    ...
                    if (!$err_m) {
                        // тут СРАЗУ выводим done, а не форму!
                        $date_start = time();
                        $date_end = $date_start + 14*24*3600;
                        $days_left = 14;
    ...
    if (in_array($_SESSION['sh_login'], ['admin','em_nsv','demo_admin'])) {
        ...
        $this->tpl($data, 'apartments', 'form_broni_pub');
    }
...
function act__order2()
{
    ...
    $stat = $apartment['status2'] ?? $apartment['a_status'] ?? '';

    // Бронь — берем из booking_* полей или из status_broni_id
    $broni = array_filter($data, fn($k) => strpos($k, 'booking_') === 0 || $k === 'broni_id', ARRAY_FILTER_USE_KEY);
    ...
    // === 0. Обработка subact (если пришел из GET)
    if ($subact && !empty($broni) && !empty($broni['broni_id'])) {
        if ($subact == 'upbroni') {
            $sa->up_broni($broni['broni_id'], 4, 'Продление пользователем');
            $date_start = time();
            $date_end = $date_start + 14*24*3600;
    ...
    if (
        ...
        $date_start = strtotime($broni['booking_date'] ?? $broni['date'] ?? '');
        $date_end = $date_start + 14 * 24 * 3600;
        $days_left = ceil(($date_end - time()) / (24 * 3600));
        if ($days_left > 0) {
            $done_message = "У вас уже есть действующая бронь на эту квартиру.<br>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$show_done_template) {
        ...
        if (in_array($_SESSION['sh_login'], ['admin','em_nsv','demo_admin'])) {
			...
			// Для шаблона немного разбиваем данные 
            $apartment = array_filter($data, fn($k) => strpos($k, 'apartment_') === 0 || in_array($k, [
                'apartament_id', 'section_id', 'floor', 'price', 'price_m', 'area', 'rooms',
                'kitchen_area', 'text', 'adress', 'plan_code', 'status', 'status2',
                'status_broni_id', 'date', 'image_pb', 'plan_type', 'image', 'area2', 'area_t'
...
//print '<br><br>';
      $sql = "
        SELECT 
            ...
            agency.*,
            apartment.*,
            homes_kvartal.title as homes_kvartal_title,
        ...
        LEFT JOIN homes AS home ON apartment.home_id = home.home_id
		LEFT JOIN homes_sections AS homes_sections ON homes_sections.section_id = apartment.section_id

		LEFT JOIN homes_kvartal AS homes_kvartal ON homes_kvartal.homes_kvartal_id = home.kvartal
        LEFT JOIN broni AS booking ON apartment.status_broni_id = booking.broni_id
        LEFT JOIN users AS user ON booking.user_id = user.id
        LEFT JOIN agency AS agency ON user.agency_id = agency.agency_id
