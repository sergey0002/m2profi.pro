...
{ 
	...
	function act__index()
	{
		...
		if($_GET['home_id'])
		{
			...
			print 'Сумарная стоимость свободных квартир : <b>'.number_format( $new_arr[2], 0, '', ' ').'</b><br/>';
			print 'Сумарная стоимость НЕ свободных квартир (продажи брони итп) : <b>'.number_format( $new_arr[3], 0, '', ' ').'</b><br/>';
			...
			if($_GET['my_summ'])
			{
				...
				print ' <br/>(необходимая, для выхода на целевой итог, cуммарная стоимость свободных квартир)<br/>';
				...
				print '<br/><br/>';
				print 'Необходимая наценка на свободные квартиры: <b>' . $nacenka.'%</b>';
				...
				print '<h2>Расчет цен для квартир с учетом наценки</h2>';
				// 
				print '<br>';
				...
				foreach($apparts  as $k=>$v)
				{
					...
					print 'Квартира <b>№'.$v['apartment_num'].'</b> Старая цена: <b>'.number_format($v['price'], 0, '', ' ') .'</b> новая цена <b>'. number_format( $rounded_price, 0, '', ' ').'</b> ';
					...
					[status_broni_date] => 
					[date] => 
					[image_pb] => https://em.m2profi.pro/sahmatka/pbplans/40/901_3_sec_2_fl_2K_67_4.svg
					[image_pb_png] => https://em.m2profi.pro/svg2png/cache/sahmatka/pbplans/40/901_3_sec_2_fl_2K_67_4.png
					[plan_type] => 0
					[image] => 0
					[area2] => 0.00
					[area_t] => 0.00
			...
			$nsz =  $itog_rounded_price - $new_arr[2];
				print 'Новая сумарная стоимость свободных квартир: <b>'.number_format( $itog_rounded_price, 0, '', ' ').'</b> - (Наценка: <b>'.number_format( $nsz, 0, '', ' ') .'</b>)</b><br/>';
				...
				print '(при применении наценки сумма может быть больше заданной, за счет округления цен на квартиры в большую сторону до ближайшей целой тысяци)<br/>';
				...
				print '<br>';print '<br>';
					print '<br><br/><br><br/>ТУТ МОЖЕТ БЫТЬ БОЛЬШАЯ КНОПКА "ПРИМЕНИТЬ НАЦЕНКУ К СВОБОДНЫМ КВАРТИРАМ';
				print '<br>';print '<br>';
			}			
