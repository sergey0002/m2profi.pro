...
function sendReminderEmail($row, &$notifications) {
    $to         = $row['e_mail'];
    $userName   = htmlspecialchars($row['name'] ?: $row['login']);
    ...
    <div class='container'>
        ...
        <div class='footer'>
            Вы получили это письмо, так как зарегистрированы на платформе em.m2profi.pro.
        </div>
    </div>
    </body>
    </html>
...
function sendTableByEmail($results, $titles) {
    ...
    $subject = 'СПИСОК ПРОСРОЧЕННЫХ БРОНЕЙ';
    ...
    <div class='container'>
        <div class='email-header'>
            <h1>Список просроченных броней</h1>
        </div>
        <div class='email-content'>
            $html_table
        </div>
        <div class='footer'>
            Это автоматическое уведомление от системы em.m2profi.pro
        </div>
    </div>
    </body>
    </html>
    ...
    if (multi_attach_mail($to, $subject, $message)) {
        error_log("✅ Таблица с просроченными бронями отправлена на $to");
        return true;
    } else {
