...
{  
	...
	function __construct()
	{
 	...
 	// БАзовый запрос  menu
	function get_base_sql($filtr_data='',$nolimit=true)
	{
		global $mysql;
		...
		$q = 'SELECT '.$this->table.'.*, rent_homes.adress as h_adress, rent_homes.trc ,
		...
		else
		{
			...
			if( $filtr_data['appointment'] =='4'){$q.=' AND (`rent_objects`.`appointment`  = "'.$filtr_data['appointment'].'") '; }
			
			elseif( $filtr_data['appointment'] ){ $q.=' AND (`rent_objects`.`appointment`  = "'.$filtr_data['appointment'].'" or `rent_objects`.`appointment`  = "0") ';  }
			...
			if( !$filtr_data['trc'] && !$filtr_data['no_trc'] )
			{
				$q.=' AND rent_homes.trc  = "0" ';
			}
			if( $filtr_data['trc']  ){ $q.=' AND rent_homes.trc  = "1" ';  }
			if( $filtr_data['no_trc']  ){ $q.=' AND rent_homes.trc  = "0" ';  }
		...
		if($filtr_data['order']=='1') // Новые
		{
			$q.=' ORDER BY `rent_objects`.`rent_objects_id` desc '; 
		}
		elseif($filtr_data['order']=='2') // Популярные
		{
		//	$q.=' ORDER BY `rent_objects`.`show_b` desc, plan DESC '; 
		}
		elseif($filtr_data['order']=='3')//Площади (по убыванию)
		{
			$q.=' ORDER BY `rent_objects`.`area` DESC  '; 
		}
		elseif($filtr_data['order']=='4') //Площади (по возрастанию)
		{
				$q.=' ORDER BY `rent_objects`.`area` ASC  '; 
		}
		...
		{
			//$q.=' ORDER BY `rent_objects`.`appointment` desc  '; 
		}
		...
		if( $filtr_data['pagep']  )
		{
			$q .= ' LIMIT  '. $filtr_data['pagep'] ;
		}
		if($nolimit==false)
		{
			// Добавляем пагинацию
			$perpage = isset($filtr_data['pp']) ? intval($filtr_data['pp']) : 10; // По умолчанию 10 записей на страницу
			$p = isset($filtr_data['p']) ? intval($filtr_data['p']) : 1; // По умолчанию первая страница
	...
	function act__pmenu()
	{
		...
		$filtr_data = $_REQUEST;
		$filtr_data['no_limit'] = true;

		// Получаем общее количество записей
		$count_query = $this->get_base_sql($filtr_data, true);
		$result = $mysql->get_arr($count_query);
		$total_count = count($result);

		// Количество записей на страницу
		$perpage = isset($filtr_data['perpage']) ? intval($filtr_data['perpage']) : 10;
		// Текущая страница
		$p = isset($filtr_data['p']) ? intval($filtr_data['p']) : 1;

		// Общее количество страниц
		$total_pages = ceil($total_count / $perpage);

		// Если нет записей или одна страница, не показываем пагинацию
		if ($total_pages <= 1) {
			return '';
		}

		// Количество ссылок на страницы слева и справа от текущей страницы
		$adjacents = 2;

		// URL без параметров страницы
		$url = $_SERVER['PHP_SELF'] . '?' . http_build_query(array_diff_key($_REQUEST, array('p' => '')));

		$pagination = '';

		// Первая страница
		if ($p > 1) {
		   // $pagination .= '<a href="' . $url . '&p=1">Первая</a> ';
		}

		// Предыдущая страница
		if ($p > 1) {
		  //  $pagination .= '<a href="' . $url . '&p=' . ($p - 1) . '">Предыдущая</a> ';
		}

		// Логика для отображения страниц
		$start = max(1, $p - $adjacents);
		$end = min($total_pages, $p + $adjacents);

		// Отображение первой страницы, если она не в диапазоне
		if ($start > 1) {
			$pagination .= '<a href="' . $url . '&p=1" class="pl">1</a> ';
			if ($start > 2) {
		...
		for ($i = $start; $i <= $end; $i++) {
			if ($i == $p) {
				$pagination .= '<span class="cp p1" >' . $i . '</span> ';
			} else {
				$pagination .= '<a href="' . $url . '&p=' . $i . '" class="pl">' . $i . '</a> ';
			}
		}

		// Отображение последней страницы, если она не в диапазоне
		if ($end < $total_pages) {
			if ($end < $total_pages - 1) {
				$pagination .= '<span class="ellipsis" class="pl">...</span> ';
			}
			$pagination .= '<a href="' . $url . '&p=' . $total_pages . '" class="pl">' . $total_pages . '</a> ';
		}

		// Следующая страница
		if ($p < $total_pages) {
		   // $pagination .= '<a href="' . $url . '&p=' . ($p + 1) . '">Следующая</a> ';
		}

		// Последняя страница
		if ($p < $total_pages) {
		  //  $pagination .= '<a href="' . $url . '&p=' . $total_pages . '">Последняя</a> ';
		}
		...
		<div class="row">
			...
			<?
				 ...
				 foreach( $sel_data as $k=>$v)
				 {
					 ...
					 $sel_arr[$v['rent_home_id']] = $v['adress'].' / '.$v['build_type']; 
				 }
			...
			print '<br><br>';
			$filed->checkbox('show_b','Доступно к бронированию (недвижимость с комиссией)',$data['show_b']); print '<br/>';
	...
	function act__jsoon_street()
	{
		...
		$arr = $mysql->get_arr('
			SELECT COUNT(*) AS c, `rent_homes`.`street` 
			FROM `rent_objects` 
			LEFT JOIN rent_homes ON rent_homes.rent_home_id = rent_objects.rent_home_id 
			WHERE `rent_objects`.`show` = "1" AND `rent_objects`.`del` = "0" 
			GROUP BY `rent_homes`.`street`
		');
		...
		$result[] = [
					...
					'name' => 'Аренда помещений', // Название улицы
		...
		foreach ($arr as $item) {
			if (!empty($item['street'])) {
				$result[] = [
					'id' => $item['street'], // Используем название улицы как ID
					'name' => trim($item['street']), // Название улицы
					'count' => $item['c'] // Количество объектов на этой улице
				];
			}
		}
	...
	function act__display_ag_ajax()
	{
		...
		$wh = declOfNum($c, array('Найден %d объект', 'Найдено %d объекта', 'Найдено %d объектов'));


		if($c){$ct = $wh;}
		else{$ct = 'Объектов не найдено';}
		?>
		...
		<div class="container rent_p" style="padding-left: 0;
    padding-right: 0;
  ">
		...
		foreach($data as $k=>$v)
		{
			...
			$this->appointment[4] = 'Земельные участки';
...
function act__display_export()
{
    global $mysql, $status_arr;
    ...
    $wh = declOfNum($c, array('Найден %d объект', 'Найдено %d объекта', 'Найдено %d объектов'));
    $ct = $c ? $wh : 'Объектов не найдено';
    ?>
    <div class="container">
        <? if ($_GET['rent_home_id'] && !empty($data)) { ?>
            <div class="p16">
                <label for="h_adress">
                    <input type="checkbox" value="1" class="form_checkbox" id="h_adress" checked="checked" style="margin-left: 0;">
                    <b>Помещения по адресу: <?= htmlspecialchars($data[0]['h_adress']) ?></b>
                </label>
            </div>
        <? } ?>
    </div>
    ...
    <div class="container rent_p" style="padding-left: 0; padding-right: 0;">
        <? if ($c): ?>
            <table class="table table-bordered" style="border-collapse: collapse; width: 100%;" border="1">
                <thead>
                    <tr>
                        ...
                        <th>Место под разгрузку</th>
                    </tr>
                </thead>
                <tbody>
                    <?
                    $appointment = [
                        0 => 'Свободное назначение',
                        1 => 'Офисное',
                        2 => 'Магазин (торговое)',
                        3 => 'Под детский центр',
                        4 => 'Земельные участки'
                    ...
                    foreach ($data as $v):
                        // Адрес
                        $addr = str_replace(['Россия,', 'Калининский район,'], '', $v['h_adress']);
                        ...
                        // Этаж
                        if (isset($v['floor']) && $v['floor'] != 0) {
                            $floor = $v['floor'] . ' этаж';
                        } elseif (isset($v['floor']) && $v['floor'] == 0) {
                        ...
                        // Назначение
                        $app_text = isset($appointment[$v['appointment']]) ? $appointment[$v['appointment']] : '—';

                        // Статус: если нет — "Свободно"
                        $status_code = $v['status'] ?? null;
                        if ($status_code === null || $status_code === '' || !isset($status_arr[$status_code])) {
                            $status_text = 'Свободно';
                        } else {
                            $status_text = $status_arr[$status_code];
                        }

                        // Мощность
                        $power = '';
                        if (ctype_digit(str_replace(',', '', $v['max_power_grid'] ?? ''))) {
                            $power = $v['max_power_grid'] . ' кВт';
                        } elseif (!empty($v['max_power_grid'])) {
                            $power = $v['max_power_grid'];
                        } else {
                            $power = '—';
                        }

                        // Кол-во помещений
                        $rooms = (!empty($v['rooms']) && $v['rooms'] > 1) ? $v['rooms'] . ' помещения' : 'Одно помещение';

                        // Разгрузка
                        $unloading = !empty($v['place_for_unloading']) ? 'Да' : 'Нет';
                        ?>
                        <tr>
                            <td><?= htmlspecialchars($addr) ?></td>
                            <td><?= htmlspecialchars($area) ?></td>
                            <td><?= htmlspecialchars($status_text) ?></td>
                            <td><?= htmlspecialchars($v['h_build_type'] ?? '—') ?></td>
                            <td><?= htmlspecialchars($floor) ?></td>
                            <td><?= htmlspecialchars($app_text) ?></td>
                            <td><?= htmlspecialchars($power) ?></td>
                            <td><?= htmlspecialchars($rooms) ?></td>
                            <td><?= htmlspecialchars($unloading) ?></td>
                        </tr>
                    <? endforeach; ?>
                </tbody>
            </table>
   ...
   ">
	...
	foreach($data as $k=>$v)
	{
		...
		// вывод шаблонов 
		 if($v['show_b'])
		 {
			$this->tpl($v,'rentobjects','cat_one_item_v2'); 
		 }
		 else
		 {
			$this->tpl($v,'rentobjects','cat_one_item_v2'); 
		 ...
		 if($i==3)
		 {
			...
			<form action="/ajax_form.php" method="post" method="POST" enctype="multipart/form-data" class="ajax_form"  style="margin-left: -15px; margin-right: -15px;">
			<div class="form_blue form_grid">

				  <h2 class="item1" >Консультация 
				специалиста</h2>
				  
					<input type="text" name="name" class="form-element item2" placeholder="Ваше имя" required>
					<input type="text" name="phone" class="form-element item3" placeholder="Телефон" required>
			  
					<button type="submit" class="btn_form_blue item4" >Заказать консультацию</button>
	...
	*/
	...
	function ajcrud_filtr()
	{
		...
		<div class="filter-item filter-item-checkbox"  > 
			...
			<div class="filter-item filter-item-checkbox"  > 
			<input type="checkbox" id="trc" name="trc" value="1" <? if($_GET['trc']){print ' checked="checked" ';} ?>> <label for="trc">ТРЦ</label><br/>
		</div>
	...
	function act__mapitems($rent_objects_id = '') 
	{
		global $mysql;

		// Получаем ID объекта из GET-параметра
		if ($_GET['oid']) {
			$rent_objects_id = $_GET['oid'];
		}
		
		 if ($_GET['bid']) {
			$rent_home_id = $_GET['bid'];
		}
	
	

		// Если передан ID объекта, возвращаем данные только для этого объекта
		if ($rent_objects_id) {
			$q = '
				SELECT 
					rh.rent_home_id,
					rh.adress AS home_adress,
					rh.lat,
					rh.lon
				FROM 
					rent_objects ro
				LEFT JOIN 
					rent_homes rh ON ro.rent_home_id = rh.rent_home_id
				WHERE 
					ro.rent_objects_id = "' . $rent_objects_id . '" 
					AND ro.del = 0
					AND rh.del = 0
			';
		} 
		...
		elseif ($rent_home_id) {
			$q = '
				...
				FROM 
					rent_homes rh
				WHERE 
					rh.rent_home_id = "' . $rent_home_id . '" 
					AND rh.del = 0
			';
		} 
		else {
			// Если ID объекта не передан, возвращаем все дома, у которых есть объекты с show=1
			$q = '
				SELECT DISTINCT
					rh.rent_home_id,
					rh.adress AS home_adress,
				...
				FROM 
					rent_homes rh
				INNER JOIN 
					rent_objects ro ON rh.rent_home_id = ro.rent_home_id
				WHERE 
					ro.show = 1
					AND ro.del = 0
					AND rh.del = 0
		...
		foreach ($data as $item) {
			if ($item['lat'] && $item['lon']) {
				$result[] = [
					'id' => $item['rent_home_id'],
					'name' => $item['home_adress'],
					'coordinates' => [$item['lat'], $item['lon']],
					'hint_content' => $item['home_adress'],
				];
			}
		}
	...
	function act__map($rent_home_id='',$href_perfix='https://em-nsk.ru/m2rent/?rent_home_id=' ) 
	{
		...
		$href_perfix='https://em.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=cardn&id=';
		...
		foreach($data as $k=>$v)
		{
			...
			{
				$cx .='<a data-wrapper=\"#ajax-wrapper\" class=\"ajfb\" style=\"color:#56A4ED;\" href=\"#\" data-url=\"'.$href_perfix.$vvv['rent_objects_id'].'\">Помещение: '.$vvv['area'].'м<sup>2</sup></a><br/>';
			...
			{	
				if($oc>1){$show_text='Все помещения';}//Все помещения
...
<?		
	...
	function act__cardn_form()
	{
		$this->tpl($data,'rentobjects','cardn_form'); 
	}
	...
	//	print_R($v);
		...
		if(ctype_digit(str_replace('.','',$v['area'])))
		{
			$area = 'Площадь – '.$v['area'].'м<sup>2</sup>';
		}
		elseif($v['area']){$area =$v['area']; }
		else{$area = 'Помещение';}
      ...
      <div class="col-md-6 p10">
        ...
        <p>
          <a href="#map" class="rent_a"><img src="https://em-nsk.ru/m2rent/images/map.svg" alt="">  на карте</a>
        </p>
		<br/>
        <p>
			<?=$v['comment']?>
...
<?
	...
	//	print_R($v);
		...
		?>	
<section>
  <div class="container-fluid rent_p " style="padding-left: 0;
    padding-right: 0;
    ">
...
<?
	...
	function act__fastorder()
	{
		...
		elseif( $_POST )
		{
			...
			$to = '89236470002@mail.ru,op@em-nsk.group,demo@d-at.ru'; //
	...
	/**
	 * Возвращает список помещений в формате JSON для внешних лендингов.
	 * Используется виджетом pvz_offers.js.
	 * Поддерживает все GET/POST фильтры через get_base_sql().
	 */
	function act__display_json()
	{
		global $mysql;

		// 1. Получаем основной SQL с применением всех фильтров
		$q = $this->get_base_sql();
		$data = $mysql->get_arr($q);

		// 2. Получаем координаты домов для маркеров карты
		//    (их нет в базовом SELECT, делаем отдельный запрос по house_ids)
		$home_ids = array_unique(array_column($data, 'rent_home_id'));
		$home_coords = [];
		if (!empty($home_ids)) {
			$ids_str = implode(',', array_map('intval', $home_ids));
			$homes = $mysql->get_arr(
				"SELECT rent_home_id, lat, lon FROM rent_homes WHERE rent_home_id IN ($ids_str)"
			);
			foreach ($homes as $home) {
				$home_coords[$home['rent_home_id']] = [
					'lat' => (float) str_replace(',', '.', $home['lat']),
					'lon' => (float) str_replace(',', '.', $home['lon']),
				];
			}
		}

		// 3. Формируем массив результатов
		$items = [];
		foreach ($data as $v) {
			// Чистим адрес (убираем лишние части)
			$adress = str_replace(array('Россия,', 'Калининский район,'), '', $v['h_adress']);

			// Директория с фото объекта (относительный путь как в display_v2)
			$rel_dir = 'rent/objects/' . $v['rent_objects_id'] . '/';
			$abs_dir = $_SERVER['DOCUMENT_ROOT'] . '/sites/em/sahmatka/' . $rel_dir;
			$photos = [];
			$photo_url = '';
			$plan_url = '';
			...
			if (is_dir($abs_dir)) {
				...
				foreach ($files as $file) {
					...
					if (in_array($ext, array('jpg', 'jpeg', 'png'))) {
						// Формируем URL как в cat_one_item_v2: /sahmatka/rent/objects/...
						$thumb_url = 'https://em.m2profi.pro/thumbs/?name=/sahmatka/' . $rel_dir . $file . '&w=600&h=400';
						$photos[] = $thumb_url;
						// Первое фото — для превью карточки
						if (!$photo_url) {
							$photo_url = $thumb_url;
						}
					}
			...
			// Планировка — как есть из БД (прямой URL)
			if (!empty($v['plan'])) {
				$plan_url = $v['plan'];
			}

			// Координаты дома
			$coords = isset($home_coords[$v['rent_home_id']]) ? $home_coords[$v['rent_home_id']] : array('lat' => 0, 'lon' => 0);

			$items[] = array(
				'id'          => (int) $v['rent_objects_id'],
				'rent_home_id'=> (int) $v['rent_home_id'],
				'h_adress'    => trim($adress),
				'adress'      => $v['adress'],
				'area'        => $v['area'],
				'floor'       => $v['floor'],
				'h_build_type'=> $v['h_build_type'],
				'max_power_grid' => $v['max_power_grid'],
				'appointment' => $v['appointment'],
				'place_for_unloading' => $v['place_for_unloading'],
				'rooms'       => $v['rooms'],
				'separate_entrance' => $v['separate_entrance'],
				'status'      => $v['status'],
				'show_b'      => $v['show_b'],
				'photo_url'   => $photo_url,       // Фото через thumbs (или пустая если нет)
				'plan_url'    => $plan_url,         // Планировка как есть (прямой URL из БД)
				'photos'      => $photos,           // Все фото через thumbs
				'lat'         => $coords['lat'],
				'lon'         => $coords['lon'],
				'detail_url'  => get_app_url() . '/sahmatka/ajax_router.php?ctr=rentobjects&act=cardn&id=' . $v['rent_objects_id'],
			);
		}

		header('Content-Type: application/json; charset=utf-8');
		header('Access-Control-Allow-Origin: *'); // Для запросов с основного домена
