...
{ 
    ...
    function act__index()
    {
        ...
        <style>
            ...
            .revenue-bar-sold {min-width:150px; background-color: rgba(220, 53, 69, 0.7); display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; padding: 5px; box-sizing: border-box; border-right: 1px solid #aaa; }
            .revenue-bar-unsold {min-width:150px; background-color: red; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; padding: 5px; box-sizing: border-box; }
    ...
    private function _generate_stats_table($title, $statuses) {
        ...
        $sql = "
            SELECT REGEXP_SUBSTR(apartaments.rooms, '[0-9]+') as roomsx,
                   COUNT(*) as c, SUM(apartaments.area) as summ_area, SUM(apartaments.price) as summ_price,
            ...
            WHERE apartaments.status2 IN ({$status_list_for_sql}) AND apartaments.rooms > 0 AND homes.`show` > 0 AND apartaments.area > 0 ";
