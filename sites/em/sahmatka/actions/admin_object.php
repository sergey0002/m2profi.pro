...
if (
    trim($_POST['neworient_1'] ?? '') !== ''
    && $_SESSION['sh_login'] == 'admin'
    && !empty($_POST['editapart'][$_GET['home']])
) {
    list($o1, $o2) = window_orient_normalize(
        (int)($_POST['neworient_1'] ?? 0),
        0
    );
    $check = window_orient_validate($o1, null);
    if ($check['ok']) {
        $o1sql = $o1 === null ? 'NULL' : '"' . (int)$o1 . '"';
        $query = 'UPDATE `apartaments` SET `window_orient_1` = ' . $o1sql . ' WHERE `home_id` = "' . (int)$_GET['home'] . '" AND ( `apartment_num` = "100000" ';
        foreach ($_POST['editapart'][$_GET['home']] as $k => $v) {
            $query .= ' OR `apartment_num` = "' . (int)$k . '" ';
        }
        $query .= ');';
...
?>
		...
		<div class="acc-body" style="display:none;">
			<label>Направление 1</label>
			<select name="neworient_1" class="inpx orient-select" style="width:45%; text-transform:none;">
				<option value="">Не задано</option>
				<?php foreach ($window_orient as $code => $label): ?>
				<option value="<?= (int)$code ?>"><?= (int)$code ?>. <?= htmlspecialchars($label) ?></option>
				<?php endforeach; ?>
			</select>
			<?php /* Направление 2 — отключено
			<label>Направление 2</label>
			<select name="neworient_2" class="inpx orient-select" style="width:45%; text-transform:none;">
				<option value="">Не задано</option>
				<?php foreach ($window_orient as $code => $label): ?>
				<option value="<?= (int)$code ?>"><?= (int)$code ?>. <?= htmlspecialchars($label) ?></option>
				<?php endforeach; ?>
			...
			*/ ?>
			<p style="font-size:12px;">Пустое поле не меняет ориентацию.</p>
			<div id="mass-orient-preview" class="window-compass-list" data-compass-base="<?= htmlspecialchars(rtrim($GLOBALS['window_orient_compass_url'] ?? '/sahmatka/images/compas/', '/') . '/') ?>"></div>
		</div>
...
$('.orient-select').on('change', function() {
  var base = $('#mass-orient-preview').data('compass-base') || '/sahmatka/images/compas/';
  var v1 = parseInt($('select[name=neworient_1]').val() || 0, 10);
  var html = '';
  if (v1 > 0 && v1 <= 8) {
    html += '<img src="' + base + v1 + '.svg" width="90" height="90" alt="" class="window-compass-img">';
  }
  $('#mass-orient-preview').html(html);
});
