...
class controller_broni
{
	...
	function get_sql( $nofiltr=0 )
	{
		...
		if(!$nofiltr)
		{
			...
			if($_SESSION['sh_login'] && !$_SESSION['adm_caption'] && $_SESSION['sh_login']!='admin' &&  $_SESSION['sh_login']!='fd' &&   $_SESSION['sh_login']!='uservip' &&  $_SESSION['sh_login']!='partner' && $_SESSION['sh_login']!='demo_admin')
			{
				$q.='AND users.login="'.$_SESSION['sh_login'].'"  '; // только его брони
			}
			//  админ агентства
			elseif($_SESSION['sh_login'] && $_SESSION['adm_caption'] && $_SESSION['sh_login']!='admin' && $_SESSION['sh_login']!='fd'   &&  $_SESSION['sh_login']!='uservip' &&  $_SESSION['sh_login']!='partner' && $_SESSION['sh_login']!='demo_admin')
			{
...
if ($result['status'] == 4) {
    ...
    if ($days_left > 1) {
	  ...
	  if ($_SESSION['sh_login'] != 'fd') {
        ...
        $url = "https://em.m2profi.pro/sahmatka/iframe_router.php?"
             . "ctr=apartments"
             . "&act=order"
	...
	if ($result['status'] == 4 && ($_SESSION['sh_login'] == 'admin' || $_SESSION['sh_login'] == 'demo_admin' ||  $result['user_id'] == $_SESSION['sh_id'])) {
