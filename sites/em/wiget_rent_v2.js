...
$('#h_adress').on('change', function() {
    ...
    sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://em.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=display_v2',0); // Грузим содержимое селек
});
...
function relate_ajax_select(th,select_id_list,formid='filtrform')
{
	...
	if(array)
	{
		...
		if(item) 
		{
			...
			sendAjaxForm( item , formid , 'https://em.m2profi.pro/sahmatka/ajax_actions.php?load='+item+'&controller='+ controller,1,'progressbar',preload(item)); // Грузим содержимое селек
...
$( "#rentsearch input,#rentsearch select" ).change(function() {
	   $('#rent_home_id').attr('value',''); // Очистка дома
       sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://em.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=display_v2',0,'progressbar','',function postload()
 {
	 /*
	 $(document).snowfall({
            flakeCount: 1,
