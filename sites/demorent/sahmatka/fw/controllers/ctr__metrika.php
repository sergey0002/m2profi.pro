 ...
 <div>
  <canvas id="myChartx" height="50px" aria-label="Hello ARIA World" role="img"></canvas>
		...
		$('#filed_group_tree').jstree({
			'core': {
			  'data': {
				"url": "https://{$GLOBALS['config']['domain']}/sites/em/sahmatka/ajax_router.php?ctr=metrika&act=jsoonajaxftree",
				'data': function(node) {
				  return {
					'id': node.id,
			...
			function add_chartline(puth)
			{
				...
				*/
				...
				//https://em.m2profi.pro/sites/em/sahmatka/ajax_router.php?ctr=metrika&act=ajax__chartdata
				$.ajax({
					 method: 'get', 
					dataType: "json",
					url: 'https://' . $GLOBALS['config']['domain'] . '/sites/em/sahmatka/ajax_router.php?ctr=metrika&act=ajax__chartdata',
					data: {id: puth},
