...
@$apartment_num=$_GET['custom_appart_id'];
@$home_id = (int) $_GET['custom_home_id'];
  
	
	
	$custom_home_id = $_GET['custom_home_id'];
	$custom_appart_id = $_GET['custom_appart_id'];
	
	
	 
	 
	$custom_result = $custom_apparts[$custom_home_id][$custom_appart_id];
...
if($_GET['action']  && $_POST[name] && $_POST[phone])
 {	 
		...
		$message .= "Дом <b>".$custom_result['homecaption']."</b> " . unit_num($apartment_num, 'dash') . " ";
	 
		 $mess =  new messages();
		 $mess->send( '89236470002@mail.ru' , unit_phrase('email_card_subject') . ' -'.$act.' '.$_POST['name'].' ' , $message );
		 $mess->send( 'op@em-nsk.group,op@em-nsk.group' , unit_phrase('email_card_subject') . ' -'.$act.' '.$_POST['name'].' ', $message );
         ...
         <div class="container-fluid" style="padding:20px;">
            <div class="row">
               <div class="col-md-12 col-xs-12" >
                  <h1 style="font-size:34px;"><b> <?=$custom_result['homecaption'];?> </b></h1>   
               </div>
			   <div class="col-md-12 col-xs-12" style="font-size:16px; text-transform: uppercase;">
				  <br/> <?= unit_label_cap('nom') ?> -   <?=$custom_result['apartment_num'];?> 
			   </div>
			 ...
			 <div class="row">
				...
				<div class="col-md-6 col-xs-12" style="text-align:center;">
                  <img src="<?=$custom_result['image_pb'];?>?x=123" style="max-height:600px; max-width:100%"  >
                </div>
			    <div class="col-md-6 col-xs-12" style="text-align:left; font-size:24px; position:relative;">
				 
				 <hr/>
				 Количество комнат - <b><?=$custom_result['rooms']?></b><br/>
				  Площадь - <b><?=$custom_result['area']?></b> м<sup>2</sup>
				 <hr/> 
...
if(1==1)
{
	 ...
	 Цена - <b><?=$custom_result['price'];?></b> 
				 ...
				 <div style="margin:10px; margin-left:0;" class="noprint">
					...
					<div id="one" style="display: none;">
						 <form action="form_order_custom.php?custom_home_id=<?=$custom_home_id?>&custom_appart_id=<?=$custom_appart_id;?>&action=mail" method="post" enctype="multipart/form-data" method="post">
	...
	<div class="row">
			...
			<div class="col-md-6">
				 <form action="form_order_custom.php?custom_home_id=<?=$custom_home_id?>&custom_appart_id=<?=$custom_appart_id;?>&action=epoteca" method="post" enctype="multipart/form-data" method="post">
	...
	<div class="tabs__content" style="padding-left: 0; font-size:16px;">
		<div class="row">
			<div class="col-md-6">
				<?= unit_phrase('book_unit') ?>  в “Энергомонтаж” можно всего за пару минут: оставьте ваш контактный телефон и наш менеджер свяжется с вами. 
				<br/>
				За качество отвечает застройщик. <br/>Заходи и живи!
			</div>
			<div class="col-md-6">
				 <form action="form_order_custom.php?custom_home_id=<?=$custom_home_id?>&custom_appart_id=<?=$custom_appart_id;?>&action=bron" method="post" enctype="multipart/form-data" method="post">
				 <input type="text" name="name" required placeholder="Ваше имя" style="padding:4px; margin:3px; border: 1px solid rgba(0, 0, 0, 0.53); width:100%; max-width:300px;" /><br/>
				 <input type="text" name="phone" required placeholder="Контактный телефон" style="padding:4px; margin:3px; border: 1px solid rgba(0, 0, 0, 0.53); width:100%; max-width:300px;"  /><br/>			
				 <div style="width:100%; max-width:300px;">
				 <input type="checkbox" name="offerta" checked="checked" value="1">	Я даю свое согласие на обработку персональных данных
