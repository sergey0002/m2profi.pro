...
{ 
	...
	array.forEach(function(item, i, arr) 
	{
		...
		sendAjaxForm( item , formid , '/sahmatka/ajax_router.php?ctr='+controller+'&act='+item +'&ajax=true', 1 , 'progressbar', predcallback , predcallback2 , postcallback ); // Грузим содержимое селек
...
sendAjaxForm( 'ид куда грузить' , 'ид формы' , 'https://xdemo.m2profi.pro/sahmatka/ajax_actions.php?load=data&controller=zapiskeys',0); // Грузим содержимое селек

sendAjaxForm( 'zapisdata' , 'filtrform' , 'https://xdemo.m2profi.pro/sahmatka/ajax_actions.php?load=data&controller=zapiskeys',0); // Грузим содержимое селек
			...
			success: function(data){
				...
				//	$('#'+result_src_id).fadeIn(3000);
					...
					if(!msg['error'])
					{
						...
						iprew = '<div class="fiprew"><img src="'+msg['link']+'" /><input type="hidden" name="'+iprew_name+'['+lastfnum+']'+'"  value="'+msg['link']+'" /></div>';
...
$(".fiprew_links_del").click(function()
{
	...
	$.ajax({
		url: '/admin/ajax_router.php?ctr=nodes&act=ajax_delimg',
		method: 'post',
		dataType: 'html',
