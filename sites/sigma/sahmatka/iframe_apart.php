...
if($_GET['action'] == 'broni'  )
 {	 
	 if($_POST && (check_access('admin') || $_SESSION['sh_login'] == 'em_nsv'  || $_SESSION['sh_login'] == 'demo_admin') )
	 {
	...
	elseif( ($_POST || $_FILES ) && !check_access('admin') &&  $_SESSION['sh_login'] != 'demo_admin' && $_SESSION['sh_login'] != 'em_nsv'  && $_SESSION['sh_id']  )
	{
 ...
 AND (apartments="'.$apartments.'" OR (apartments_num="'.$apartments_num.'" AND apartments="0") )
		...
		if($stat!='' && $stat!='2' && $stat!='5' && $stat!='0' ){print '<h2 style="color:red">Ошибка бронирования ' . unit_label('nom') . ' уже забронированы другим пользователем</h2>';  $err_m[]=unit_phrase('already_booked');}
		else
		{
		...
		else
		{
		 ...
		 if($_FILES['anket']['type'])
		 {
		  ...
		  {
		  ?>
		  <h2 style="color:#000; text-align:center;"><?= unit_phrase('booked_label') ?></h2>
		...
		// вложения файлов
		$message = unit_phrase('email_booking') . " \r\n <br/>";
		$message .= "Заявка поступила от пользователя - <b>".$_SESSION['sh_name'].'</b> Представителя агентства - <b>'.$_SESSION['ucaption']."</b>\r\n </b><br/> ";
		$message .= "Дом <b>".$homes[$home_id]['caption']."</b> секция-<b>".$section_id."</b> Этаж-<b>". $floor."</b> " . unit_num($apartments_num, 'dash') . " ";
...
?>
         ...
         <div class="container-fluid">
            <div class="row">
			   ...
			   <div class="col-md-12 col-xs-12" style="text-align:center; font-size:22px;">
				...
				</span> Этаж - <span style="color:#00CDAD; font-weight:bold;"><?=$result['floor'];?></span> <?= unit_label_cap('nom') ?> - <span style="color:#00CDAD; font-weight:bold;"><?=$result['apartment_num'];?></span>
			   <hr/>
			   </div>
	...
	<?
	if( (check_access('admin')  || $_SESSION['sh_login'] == 'em_nsv' || $_SESSION['sh_login'] == 'demo_admin')   )
	{
	?>
	<form action="iframe_apart.php?home_id=<?=$home_id?>&section_id=<?=$result['section_id']?>&floor=<?=$result['floor']?>&apartment_num=<?=$apartment_num;?>&apartments=<?=$apartments;?>&action=broni" method="post" enctype="multipart/form-data" method="post">
		...
		?>
		  <option value="4" <? if(4==$stat){ ?> selected="selected" <? }?>>Забронирована</option>
 		  <option value="6" <? if(6==$stat){ ?> selected="selected" <? }?>><?= unit_phrase('contractor') ?></option> 
	...
	elseif(!check_access('admin') &&  $_SESSION['sh_login'] != 'demo_admin' && $_SESSION['sh_login'] !='em_nsv'  ) // Админ агентства или агент
	{
		?>
...
if( $stat=="2" || !$stat   ) 
{
	...
	<br/>
<span style="font-size:12px; color:#ff0000;">Дни приема актов: понедельник, вторник, четверг с 9.30 до 14.00
...
<?
 ...
 if(1==2 && check_access('admin') )
 {
print 'История бронирования'	 ;
 ob_start();
...
if( $_GET[home] )
{
	$q.=' AND broni.home_id="'.$_GET['home_id'].'" ';
}
...
elseif(  $result['bstatus']==6 ){ print unit_phrase('contractor');} 
...
elseif(  $result['status2']==6 ){ print unit_phrase('contractor');} 
