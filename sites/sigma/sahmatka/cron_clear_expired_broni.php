...
function sendReminderEmail($row, &$notifications) {
	...
	$subject = unit_phrase('bron_expires') . " д.{$home} " . unit_num($row['apartment_num'], 'hash');
	/*
	...
	$youBookedText = unit_phrase('you_booked');
	$willBeFreedText = unit_phrase('will_be_freed');
    print $message = "
    <html>
    <head>
    <style>
    ...
    <div class='container'>
        ...
        <div class='content'>
            ...
            <p>{$youBookedText} <strong>№{$apartment}</strong> в доме <strong>«{$home}»</strong><br>Дата бронирования: <strong>{$bronDate}</strong>.</p>
            <p><strong>Внимание!</strong> Срок действия брони истекает <strong>{$GLOBALS['label']}</strong>.</p>
            <p>Если вы планируете продлить бронь — пожалуйста, сделайте это заранее, иначе {$willBeFreedText}.</p>
        </div>
        <div class='footer'>
