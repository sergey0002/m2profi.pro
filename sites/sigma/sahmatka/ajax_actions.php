	...
	?>
		 <div class="m2catalog_item">
				 ...
				 <div class="m2catalog_item_order_frame">
					...
					<a class="m2catalog_item_order iframe" href="<?=htmlspecialchars('https://' . $GLOBALS['config']['site_subdomain'] . '.m2profi.pro/sahmatka/form_order.php', ENT_QUOTES, 'UTF-8')?>?home_id=<?=$result['home_id']?>&apartment_num=<?=$result['apartment_num']?>&apartments=<?=$result['apartments']?>">Забронировать</a> 
				 </div>
...
elseif($_GET['action']=='keys')
{
	?>
		...
		<form action="#">
			...
			<input type="text" placeholder="<?= unit_label_cap('nom') ?>">
			<input class="phone-in" type="tel" placeholder="Телефон">
			<input type="text" placeholder="ФИО">
			<button class="btn btn_notbdrs callback-form__btn">Сохранить измененения<i></i></button>
		</form>
