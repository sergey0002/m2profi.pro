...
{  
	...
	function printspace_disp($data='')
	{
		...
		{
			$href   = 'https://hotel.m2profi.pro/sahmatka/iframe_router.php?ctr=parking_spaces&act=order&id='.$data['parking_space_id'].'';
		...
		elseif( $_SESSION['sh_login'] &&  $_SESSION['agency_id'] == 92 ) // Все залогиненые - кликть можно на все парковочные места карточка должна меняться в зависимости от прав
		{
			if($data['status_broni_id'])
			{
				if($data['login']=='admin'){   $data['caption']='';}
		...
		elseif( $_SESSION['sh_login'] ) // Все залогиненые - кликть можно на все парковочные места карточка должна меняться в зависимости от прав
		{
			if($data['status']!=2){$price='';}else{$price.='<br/>';}
			$href   = 'https://hotel.m2profi.pro/sahmatka/iframe_router.php?ctr=parking_spaces&act=order&id='.$data['parking_space_id'].'';
			?>
		...
		{
			if($data['status']!=2){$price='';}else{$price.='<br/>';}
			$href   = 'https://hotel.m2profi.pro/sahmatka/iframe_router.php?ctr=parking_spaces&act=broni&id='.$data['parking_space_id'].'';
			?>
		...
		<div style="padding-right:0; padding-left:0; min-height:auto;     margin-bottom: 0;    margin-top: 15px;" class="page-header">	 
			 ...
			 foreach($adress_arr as $k=>$v)
			 {
				...
				if($v['parking_building_id'] == $parking_building_id){$actclass=' mdefth '; }else{$actclass='';}
					if( ($v['show']==2  ) && check_access('admin') ) // Админу показывать все
					{
						?>
					...
					elseif(  $v['show']==3  &&  ( $_SESSION['agency_id'] == 92 || check_access('admin')) ) // ТОЛЬКО ОТДЕЛУ ПРОДАЖ
					{
						?>
		...
		<hr style="margin-top: 12px; " class="nomobile">
		 ...
		 if(!$parking_building_id)
		 {
			...
			<div class="row">
				...
				foreach($adress_arr as $k=>$v)
				{
					if( $v['show']==0 && check_access('admin') ){ $opt = 'opacity: 0.4; '; }else{ $opt =''; }
					$v['opt'] = $opt;
					...
					elseif(  $v['show']==3  &&  ( $_SESSION['agency_id'] == 92 || check_access('admin') || $_SESSION['sh_login'] == 'fd' || $_SESSION['sh_login'] == 'em_nsv') ) // ТОЛЬКО ОТДЕЛУ ПРОДАЖ
					{
						$this->tpl($v,'parking_floors','catalog_card');
					}
					elseif( $v['show']==1  ) // ПОКАЗЫВАТЬ ВСЕМ!
