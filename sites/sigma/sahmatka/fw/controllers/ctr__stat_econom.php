...
;
    ...
    line-height: 2em;
       ...
       // $this->_render_filters(); 
        ...
        $table_sold_html = $this->_generate_stats_table(unit_phrase('sold_table'), [3]);
        $table_unsold_html = $this->_generate_stats_table(unit_phrase('unsold_table'), [0, 2, 4, 5, 6]);
        $table_booked_html = $this->_generate_stats_table(unit_phrase('booked_table'), [4]);
    ...
    <div class="financial-summary">
        ...
        <div class="revenue-bar">
            <div class="revenue-bar-sold"  style="     background-color: #888; width: <?= $sold_width_percent ?>%;">
                <div class="bar-label">Продано <?= $total_sold_count ?> <?= unit_abbrev() ?></div>
                <div class="bar-value">на <?= number_format($total_sold_price, 0, ',', ' ') ?>  </div>
            </div>
            <div class="revenue-bar-unsold" style="width: <?= $unsold_width_percent ?>%;">
                <div class="bar-label">Осталось <?= $potential_unsold_count ?> <?= unit_abbrev() ?></div>
        ...
        <div class="plan-value" style="margin-top: 5px;">
            Прогнозируемая сумма по комплексу: <span><?= number_format($total_potential_revenue, 0, ',', ' ') ?></span>
        </div>
