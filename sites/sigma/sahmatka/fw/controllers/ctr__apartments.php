...
*/
	...
	function act__data()
	{
		...
		if(!$this->data_arr)
		{
			?><div style="width:100%; text-align:center;"><?= unit_phrase('search_not_found') ?></div><?
		}
		// print_R( $this->data_arr);
	...
	?>
				...
				<div class="m2catalog_item_content_frame">
						...
						<span style="    font-size: 12px; color: #EEE;"><b><?= unit_phrase('room_numbers') ?></b> <?=$result['anums']?>   </span> <br/>
...
<a class="m2catalog_item_order iframe" href="https://hotel.m2profi.pro/sahmatka/iframe_router.php?ctr=apartments&act=card&home_id=<?=$result['home_id']?>&apartment_num=<?=$result['apartment_num']?>&apartments=<?=$result['apartments']?>">Забронировать</a> 
				</div>
	...
	public function act__history()
	{
		...
		if (!$apartament_id) {
			echo "<div class='warn'>" . unit_phrase('id_not_set') . "</div>";
			return;
		}
		...
		if (!$rows || !count($rows)) {
			echo "<div class='info'>" . unit_phrase('no_bookings_yet') . "</div>";
			return;
		}

		echo "<div class='broni-history'>";
		echo "<h3>" . unit_phrase('history_title') . " #{$apartament_id}</h3>";
...
function act__order()
{
    ...
    if ($subact && !empty($broni) && !empty($broni['b_id'])) {
        if ($subact == 'upbroni') {
            ...
            $done_message = "Бронь отменена. " . unit_phrase('available_again') . ".";
        } else {
    ...
    $is_user = !(check_access('admin') || in_array($_SESSION['sh_login'], ['em_nsv','demo_admin']));
    ...
    if (
        ...
        if ($days_left > 0) {
            $done_message = unit_phrase('existing_booking') . ". <br> Цена брони - ".$data['b_price']." <br>
                Она будет действовать ещё <b>$days_left</b> дней — до <b>".date('d.m.Y', $date_end)."</b>.";
            $show_done_template = true;
            $show_form = false;
        } else {
            $success = unit_phrase('booking_expired') . ".";
            $show_form = true;
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$show_done_template) {
        ...
        if (check_access('admin') || in_array($_SESSION['sh_login'], ['em_nsv','demo_admin'])) {
            ...
            add_log(unit_phrase('status_changed_admin'));
            $success = unit_phrase('status_changed') . "!";
        ...
        elseif ($show_form && $_SESSION['sh_id']) {
            if ($stat != '' && $stat != '2' && $stat != '5' && $stat != '0') {
                $err_m[] = unit_phrase('already_booked');
            } else {
                ...
                if (!$err_m) {
                    ...
                    if (!$err_m) {
                        ...
                        $done_message = unit_phrase('booked_success') . "!<br>Ваша бронь активна ещё <b>$days_left</b> дней — до <b>".date('d.m.Y', $date_end)."</b>.";
    ...
    if (check_access('admin') || $_SESSION['sh_login'] == 'fd' || $_SESSION['sh_login'] == 'director') {
...
function act__broni_history($home_id, $apartment_num)
{
    ...
    $status_map = [0=>'Свободна',2=>'Свободна',3=>'Продана',4=>'Забронирована',5=>'Бронь застройщика',6=>unit_phrase('contractor')];
...
{ 
	...
	try {
		...
		*/
		...
		if (!$fw_mailer->send($recipients, 'Заявка EM-NSK.RU - публичная карточка ' . unit_label('gen') . ' ', $message)) {
