...
{  
	...
	{
		global $mysql;
		$arr = $mysql ->get_arr(' SELECT count(*) as c, `rent_homes`.`street` FROM `rent_objects` LEFT JOIN rent_homes ON rent_homes.rent_home_id = rent_objects.rent_home_id WHERE    `rent_objects`.`show`  = "1" AND   `rent_objects`.`del`  = "0" AND rent_homes.del = "0"   GROUP BY `rent_homes`.`street`   ');
	...
	function act__jsoon_street()
	{
		...
		$arr = $mysql->get_arr('
			...
			FROM `rent_objects` 
			LEFT JOIN rent_homes ON rent_homes.rent_home_id = rent_objects.rent_home_id 
			WHERE `rent_objects`.`show` = "1" AND `rent_objects`.`del` = "0" AND rent_homes.del = "0" 
...
$('#h_adress').on('change', function() {
    ...
    sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://hotel.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=display_ag_ajax&sale=<?=$_GET['sale']?>',0); // Грузим содержимое селек
});
...
sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://hotel.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=display_ag_ajax&sale=<?=$_GET['sale']?>',0); // Грузим содержимое селек
// Улицы
sendAjaxForm( 'street' , 'rentsearch' , 'https://hotel.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=sel_street',0); // Грузим содержимое селек
 
// ЗАГРУЗКА ДАННЫХ ПРИЛЮБОЙ ОБРАБОТКЕ ФОРМЫ!
$( "#rentsearch_ag input,#rentsearch_ag select" ).change(function() {
	   $('#rent_home_id').attr('value',''); // Очистка дома
	 sendAjaxForm( 'rent_search_result' , 'rentsearch_ag' , 'https://hotel.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=display_ag_ajax&sale=<?=$_GET['sale']?>',0,'progressbar','',function postload() {}); // Грузим содержимое селек
   // $('#maprentobjects').hide();
});
...
?>
	...
	*/
	...
	function act__map($rent_home_id='',$href_perfix='https://em-nsk.ru/m2rent/?rent_home_id=' ) 
	{
		...
		$href_perfix='https://hotel.m2profi.pro/sahmatka/ajax_router.php?ctr=rentobjects&act=cardn&id=';
	...
	//	print_R($v);
      ...
      <div class="col-md-6 p10">
		...
		if($v['status'] == 2 || !$v['status'] )
					{
						?>	
					 <a href="https://hotel.m2profi.pro/sahmatka/iframe_router.php?ctr=rentobjects&act=order&id=<?=$v['rent_objects_id']?>">
	   ...
	   <?
 		...
 		foreach($files as $file) 
		{
			...
			{		
				...
				{
					//https://hotel.m2profi.pro/thumbs/?name=sahmatka/logo90.jpeg
