...
{ 
	...
	function get_graficx( $future = false , $pom = false , $dkp = false , $sh = 0 )
	{
		...
		$q = 'SELECT zapis.* FROM zapis WHERE `date` > "2024-01-01" AND del="0" ';
		$data = $mysql->get_arr($q);
...
############## ЗАПИСЬ
	...
	function act__sel_section_zapis()
	{
		...
		{ 
			  ...
			  // if($_POST['home_id']=='49' && !$_REQUEST['admin'] && $v['section_id']=="3"){continue;}
			?><option value="<?= $v['section_id'] ?>">Секция №<?= $v[
  'section_id'
] ?> </option><?
		}
	}
		...
		{  
			 ...
			 { 
				...
				$iskl =true; // Исключения (true выводим) 
				 ...
				 // --- НОВОЕ УСЛОВИЕ ПО СЕКЦИИ 3 И ДАТЕ 03.02.2025 ---
				// --- НОВОЕ УСЛОВИЕ ---
                $limit_date = strtotime('02.02.2026');
                $current_date = strtotime($fd);
                $section = $_REQUEST['section_id'];

                $iskl = true; 

                if ($section == 3) {
                    // Для 3-й секции только даты ПОСЛЕ 03.02.2026
                    if ($current_date <= $limit_date) { $iskl = false; }
                } else {
                    // Для остальных только даты ДО 03.02.2026 (включительно)
                    if ($current_date > $limit_date) { $iskl = false; }
                }
                // ----------------------
		...
		foreach($this->graficx_i_date[$date] as $k=>$v) // Ид групп полуачем
		{
			...
			?><div class="<?= $grclassx ?>"><?
				...
				foreach($this->graficx_i_times[$k] as $k2=>$v2)
				{
					...
					if( $this->graficx_i_dkp[$k][$k2] ){ print   ' <b> - ДКП</b>'; }
	...
	function act__edit_daygroup()
	{
		...
		if( $_POST )
		{
			...
			{ 
				...
				if($_POST['times_dkp'][$k]) { $date_insert['dkp'] = $_POST['times_dkp'][$k]; }else{ $date_insert['dkp']=0;}
		...
		if( $id )
		{
			...
			<?
				...
				foreach($this->time_arr as $k=>$v)
				{
					...
					$filed->checkbox('times_dkp['.$k.']','ДКП',$data_times_dkp[$k]);
					?>
					</div>
		...
		else
		{
			...
			if(  $free  )
			{
				...
				{ 	 
					...
					//multi_attach_mail('s89236470002@yandex.ru', 'Запись на выдачу ключей - дом:'.$this->homes_arr[$data['home_id']]['title'].' кв.:'.$data['apartment_num'].' дата:'.$_POST['date'].' время:'.$data['time'], $con, 'admin@m2profi.pro', 'em-nsk.ru');
					
					multi_attach_mail('89236470002@mail.ru', 'Запись на выдачу ключей - дом:'.$this->homes_arr[$data['home_id']]['title'].' ' . unit_num($data['apartment_num'], 'colon') . ' дата:'.$_POST['date'].' время:'.$data['time'], $con, 'sdsasd@m2profi.pro', 'em-nsk.ru');
					
					multi_attach_mail($data['email'], 'Запись на выдачу ключей - дом:'.$this->homes_arr[$data['home_id']]['title'].' ' . unit_num($data['apartment_num'], 'colon') . ' дата:'.$_POST['date'].' время:'.$data['time'], $con, 'admin@m2profi.pro', 'em-nsk.ru');
					multi_attach_mail('op15@em-nsk.group', 'Запись на выдачу ключей - дом:'.$this->homes_arr[$data['home_id']]['title'].' ' . unit_num($data['apartment_num'], 'colon') . ' дата:'.$_POST['date'].' время:'.$data['time'], $con, 'admin@m2profi.pro', 'em-nsk.ru');
...
#form_progressbar{  
    ...
    font-size: 20px;
		 ...
		 <form method="post" action="https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapiskeys&act=zapisformx" id="zapisform">
		 <div id="form_progressbar">Загрузка...<br/><img src="http://xdemo.m2profi.pro/sahmatka/loader.gif"></div>
     ...
     'pom'
			...
			<select class="input_edit" name="apartament_num" id="apartament_num"  class="suk_form" data-bl="3" required="required" disabled><option value=""><?= unit_label_cap('nom') ?></option> </select> <br/>
...
(function($) {
	...
	//
		...
		$(document).ready(function() {
			...
			$("#home_id").fwloadx("https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapisx&act=sel_home_zapis",function() {
			...
			{
				if ($(this).val()){
					$("#home_id").fwloadx("https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapisx&act=sel_home_zapis");
				}
				else{
			...
			{
				if ($(this).val()){
					$("#section_id").fwloadx("https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapisx&act=sel_section_zapis");
				}
				else{
					blocked_child_select($(this).attr('data-bl'));
				}
			...
			{
				if ($(this).val()){
					$("#apartament_num").fwloadx("https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapisx&act=sel_apartament_zapis");
				}
				else{
					blocked_child_select($(this).attr('data-bl'));
				}
			...
			{
				if ($(this).val()){
					$("#date").fwloadx("https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapisx&act=sel_date_zapisx");
				}
				else{
					blocked_child_select($(this).attr('data-bl'));
				}
			...
			{
				if ($(this).val()){
					$("#time").fwloadx("https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=zapisx&act=sel_time_zapisx");
				}
				else{
					blocked_child_select($(this).attr('data-bl'));
				}
