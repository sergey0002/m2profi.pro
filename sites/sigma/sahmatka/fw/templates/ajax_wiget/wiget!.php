...
// <script> 
	...
	if(!window.$)
	{
		...
		//script.src = "https://xdemo.m2profi.pro/sahmatka/template/default/libs/jquery-3.3.1/jquery-3.3.1.min.js";  
	...
	*/
	function relate_ajax_select(th,select_id_list,formid='filtrform')
	{
		...
		if(array)
		{
			...
			if(item) 
			{
				...
				sendAjaxForm( item , formid , 'https://xdemo.m2profi.pro/sahmatka/ajax_actions.php?load='+item+'&controller='+ controller,1,'progressbar',preload(item)); // Грузим содержимое селек
	...
	// Начальная загрузка данных 
	 sendAjaxForm( 'street' , 'rentsearch' , 'https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=<?=$v['ctr']?>&act=sel_street',0); // Грузим содержимое селек

	 
	 
	// Начальная загрузка данных 
 	sendAjaxForm( 'rent_search_result' , 'rentsearch' , 'https://xdemo.m2profi.pro/sahmatka/ajax_router.php?ctr=<?=$v['ctr']?>&act=<?=$v['act']?>',0); // Грузим содержимое селек
