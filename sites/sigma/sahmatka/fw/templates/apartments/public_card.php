...
.mdl-tabs__item.active {
  ...
  <div class="mdl-inner" id="printableArea">
    ...
    <a href="#" class="mdl-back" onclick="window.parent.postMessage('close-fancybox', '*'); return false;"><?= unit_phrase('back_to_list') ?></a>
          <div class="mdl-dnd">
      ...
      <div class="mdl-title no-print"><?= unit_phrase('booking_title') ?></div>
      <div class="mdl-prm"><?=$data['data']['rooms']?>К | <?=$data['data']['area']?> м<sup>2</sup></div>
    </div>
    ...
    <div class="mdl-foot">
      <div class="mdl-desc">
        ...
        <p><?= unit_label_cap('nom') ?>: №<?=$data['data']['apartment_num']?></p>
      </div>
	  ...
	  {
		  ?>
		  <form action="<?=htmlspecialchars('https://' . $GLOBALS['config']['site_subdomain'] . '.m2profi.pro/sahmatka/ajax_router.php?ctr=apartments&act=card_ajaxform', ENT_QUOTES, 'UTF-8')?>" method="POST"  data-fp-id="public_card" class="formprotect mdl-form no-print  ">
