    ...
    <div class="row">
        ...
        <div class="col-md-12 col-xs-12 text-center" style="font-size:22px;">
            <p>
                ...
                <?= unit_label_cap('nom') ?> - <span style="color:#00CDAD; font-weight:bold;"><?= htmlspecialchars($apartment['apartment_num'] ?? '') ?></span>
            </p>
            <hr/>
        </div>
    </div>
    ...
    <div class="row mt-4">
            ...
            <?php if ($old_price > 0 && $new_price > 0): ?>
                <?php if ($new_price != $old_price): ?>
                    <div class="broni-price-alert" style="color:red; font-weight:bold; text-align:center; width:100%;">
                        <b>Внимание!</b> <?= unit_phrase('price_changed_actual') ?>.<br>
						<?= unit_phrase('booked_at_price') ?>: <b><?=$old_price_fmt?></b><br/>
        ...
        <?php else: ?>
            <div class="text-center mt-4">
                <p><?= unit_phrase('no_active_booking') ?>.</p>
            </div>
        <?php endif; ?>
    </div>
</div>