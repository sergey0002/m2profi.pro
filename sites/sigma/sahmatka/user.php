...
if( $_GET['url'] )
{ 
	$_GET['url'] = 'Location: https://' . $GLOBALS['config']['site_subdomain'] . '.m2profi.pro/' . ltrim((string)$_GET['url'], '/');
	header($_GET['url']);
	exit();
}
...
include('gantt.php');
 ...
 if(!$_GET['action']){$_GET['action']='objects';}
...
<script src="fancybox-3.0/dist/jquery.fancybox.min.js"></script>
		...
		<script src="template/default/js/proposal_manager.js"></script>
<link rel="stylesheet" href="template/default/css/proposal_elements.css">
 ...
 afterClose: function () { // USE THIS IT IS YOUR ANSWER THE KEY WORD IS "afterClose"
	...
	beforeLoad: function() {   
      ...
      //не выполнять
		...
		<?
		if(($_SESSION['agency_id'] == "92" || check_access('admin') || $_SESSION['sh_login'] == 'fd' || $_SESSION['sh_login'] == 'demo_admin'  ) ||1==1)
		{
			if($_GET['sdan'])
			{
		...
		?>
		 	          ...
		 	          <form id="obj_nav_form" method="GET" action="user.php" class="mobilenav" name="autosubmit_select"  >
						<div class="objects-head-nav__select"  >
							...
							?>
							<?
							if($_SESSION['agency_id'] == "92" || check_access('admin')  || $_SESSION['sh_login'] == 'demo_admin')
							{
								if(isset($_GET['sdan']))
								{
								?>
...
elseif(check_access('admin') || $_SESSION['sh_login'] == 'director' || $_SESSION['sh_login'] == 'fd' || $_SESSION['sh_login'] == 'demo_admin')
{
...
{	 
 ...
 {
		//object_menu();
		if($_GET['home']  && !$_GET['apartments'] )
		{
			include('actions/admin_object.php');
		}
 ...
 elseif($_GET['action']=='lp') // 
 {
			include('actions/lp.php');
 }
 
elseif($_GET['action']=='pr') // 
 {
			include('actions/pr.php');
 }
 elseif($_GET['action']=='pr_edit') // 
 {
			include('actions/pr_edit.php');
 }
 elseif($_GET['action']=='pos') // 
 {
			include('actions/pos.php');
 }
  elseif($_GET['action']=='gh') // 
 {
			include('actions/gh.php');
 }
...
if(!check_access('admin') && $_SESSION['sh_login'] != 'fd' &&  $_SESSION['sh_login'] != 'demo_admin' && $_SESSION['adm_caption'] )
{
 ...
 if($_GET['action']=='objects2') // Агентства (администратор)
{
 ...
 elseif(!check_access('admin')  && $_SESSION['sh_login'] != 'fd' && $_GET['action']=='objects') // Обьекты (администратор)
 {
		//object_menu();
		if($_GET['home']  && !$_GET['apartments'] )
		{
...
elseif($_SESSION['sh_login'] && !$_SESSION['adm_caption'] && !check_access('admin') && $_SESSION['sh_login']!='demo_admin' && $_SESSION['sh_login'] != 'fd' &&  $_SESSION['sh_login']!='uservip' &&  $_SESSION['sh_login']!='partner' )
{
...
if($_SESSION['agency_id'] == "92" && !check_access('admin') && $_SESSION['sh_login'] != 'fd'  && $_SESSION['sh_login'] != 'director' )
{
