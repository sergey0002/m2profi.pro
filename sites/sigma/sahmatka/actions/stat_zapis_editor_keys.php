...
<section class="section-stat">
	...
	<div style="display:none;   padding-right:0; padding-left:0; min-height:auto;     margin-bottom: 0;    margin-top: 15px;" class="page-header">	 
		...
		for( $m=1; $m<=12; $m++ )
		{
			...
			if(strlen($m)<2){$m = '0'.$m;} 
			print ' / <a href="https://xdemo.m2profi.pro/sahmatka/user.php?action=zapis_editor_keys&month='.$m.'" '.$st.'>'.$m.'</a>  ';
		}
		...
		print ' / <a href="https://xdemo.m2profi.pro/sahmatka/user.php?action=zapis_editor_keys&month=01&year='.$y.'" '.$st.'>01-'.$y.'</a>  ';
		...
		//	$data_str[31]['caption'] = '816';
			...
			foreach( $data as $k => $v )
			{
			 ...
			 ?>
			 <form action="https://xdemo.m2profi.pro/sahmatka/user.php?action=zapis_editor_keys&month=<?=$_GET['month']?>&year=<?=$_GET['year']?>" method="POST" id="ajax_form">
			 <?
