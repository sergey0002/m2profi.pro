...
$base_url = '/sahmatka/user.php?action=gh'; 
...
if (!$builder || !isset($houses_data[$region][$builder])) {
    $keys = array_keys($houses_data[$region]);
    $builder = $keys[0];
}

$display_houses = $houses_data[$region][$builder];
...
<section class="section-objects">
    <div class="container mobc">
        ...
        <div class="page-header" style="margin-bottom:0;">
            ...
            <div class="page-header__title" style="font-size: 41px;">Готовые дома</div>
        ...
        <div style="padding-right:0; padding-left:0; min-height:auto; margin-bottom: 0;" class="page-header">
            <br>
            <ul class="mmenu">
                <?php foreach ($houses_data[$region] as $b_name => $houses): ?>
                    <li style="padding:0;">
            ...
            <form id="obj_nav_form" method="GET" action="/sahmatka/user.php" class="mobilenav" name="autosubmit_select">
                <input type="hidden" name="action" value="houses">
        ...
        <div class="objects">
            <div class="row">
                <?php foreach ($display_houses as $index => $house): ?>
                    <?php 
                    // Генерация уникального ID для дома
                    $unique_id = 'house_' . md5($region . '_' . $builder . '_' . $house['name'] . '_' . $index);
                    ?>
                    <div class="col-sm-6 col-md-6 col-lg-4 col-xl-3">
                        <div class="object">
                            ...
                            $icon = $is_added ? '✓' : '+';
                            ?>
                            <div class="add-to-proposal <?= $active_class ?>" data-object-type="house" data-object-id="<?=$unique_id?>">
                                <i class="icon-add-to-proposal"><?= $icon ?></i>
                                <span class="added-message" style="display:none;">Добавлено в предложение</span>
                            </div>
                            <div class="object__title"><?= htmlspecialchars($house['name']) ?></div>
                            <div class="object__pict">
                                <img src="<?= htmlspecialchars($house['img']) ?>" alt="<?= htmlspecialchars($house['name']) ?>" style="height:200px; object-fit:cover;">
                                <div class="object__info">
                                    <div class="object__status object__status_sale">
                                        <?= number_format($house['price'], 0, '.', ' ') ?> ₽
                                    </div>
                                    <div style="font-size:12px; color:#666; margin-top:5px; line-height: 1.4em;">
                                        Застройщик: <?= htmlspecialchars($builder) ?><br>
                                        <span style="color:#333;"><?= htmlspecialchars($house['descr']) ?></span>
                                    </div>
                                </div>
                            </div>
                            <a href="#" class="object__btn btn btn_arrow">Подробнее<i></i></a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
