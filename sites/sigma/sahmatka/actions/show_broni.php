...
class controller_broni
{
	...
	function action__delete_own()
	{
		global $sa;

		$broni_id = intval($_GET['broni_id']);

		if (!$broni_id) {
			$sa->mess("Ошибка: неверный ID");
			return;
		}

		// Получаем бронь
		$data = $sa->sql->get_for_key('broni', 'broni_id', $broni_id);

		if (!$data) {
			$sa->mess("Ошибка: бронь не найдена");
			return;
		}
		...
		if ($data['user_id'] != $current_user_id) {
			$sa->mess("Ошибка: вы можете удалить только свою бронь");
			return;
		}

		// Вызываем up_broni — с user_id текущего пользователя (не админ)
		$sa->up_broni($broni_id, 2, 'Отмена пользователем');
	}
	function filtr_form()
	{
		$this->get();
	...
	function get_sql( $nofiltr=0 )
	{
		...
		users.login,users.id as user_id, users.name, users.phone, users.e_mail, /* текущая бронь */
		...
		if(!$nofiltr)
		{
			...
			if($_SESSION['sh_login'] && !$_SESSION['adm_caption'] && !check_access('admin') &&  $_SESSION['sh_login']!='fd' &&   $_SESSION['sh_login']!='uservip' &&  $_SESSION['sh_login']!='partner' && $_SESSION['sh_login']!='demo_admin')
			{
				$q.='AND users.login="'.$_SESSION['sh_login'].'"  '; // только его брони
			}
			//  админ агентства
			elseif($_SESSION['sh_login'] && $_SESSION['adm_caption'] && !check_access('admin') && $_SESSION['sh_login']!='fd'   &&  $_SESSION['sh_login']!='uservip' &&  $_SESSION['sh_login']!='partner' && $_SESSION['sh_login']!='demo_admin')
			{
		...
		<table>
			<thead> 
				<tr>
							...
							<th><a href="#"><b><?= unit_label_cap('nom') ?></b></a></th>
...
if ($result['status'] == 4) {
    // можно вынести в начало файла, если используется глобально
    $days_passed = $result['da'];
    $days_left = $BRONI_AUTO_REMOVE_DAYS - $days_passed;

    if ($days_left > 1) {
        echo 'До снятия ' . $days_left . ' дней';
    } elseif ($days_left == 1) {
        echo 'До снятия 1 день';
    } elseif ($days_left == 0) {
        echo 'Бронь истекает сегодня';
    } else {
        echo 'Бронь просрочена на ' . abs($days_left) . ' дней';
    }

 
	
	  if ($_SESSION['sh_login'] != 'fd') {
        // --- СФОРМИРОВАННАЯ ССЫЛКА на продление ---
        $url = "https://xdemo.m2profi.pro/sahmatka/iframe_router.php?"
             . "ctr=apartments"
             . "&act=order"
             . "&home_id=" . urlencode($result['home_id'])
             . "&apartment_num=" . urlencode($result['apartment_num'])
             . "&apartments=" . urlencode($result['apartments'])
             . "&broni_id=" . urlencode($result['broni_id'])
             . "&update_broni=1";
        echo '<br/><br/><a href="'.$url.'" class="iframe_r"><b>Обновить </b></a><br/><br/>';
    }
	...
	if ($result['status'] == 4 && (check_access('admin') || $_SESSION['sh_login'] == 'demo_admin' ||  $result['user_id'] == $_SESSION['sh_id'])) {
		//echo ' <a href="?action=show_broni&act=delete_own&broni_id=' . $result['broni_id'] . '" onclick="return confirm(\'Отменить бронь?\')"><b>Снять</b></a>';
	}
	 ...
	 <tfoot>
			 <tr>
				<td colspan="7" style="text-align:left;" align="left"><b><?= unit_phrase('count_label') ?> <?=$summ_arr['c']?></b></td>
...
if ($_GET['act'] == 'delete_own') {
    $broni->action__delete_own();
}
