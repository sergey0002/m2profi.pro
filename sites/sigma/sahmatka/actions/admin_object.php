 ...
 if(trim($_POST[newprice]) &&  check_access('admin') )
 {
 ...
 if(trim($_POST[newplan]) &&  check_access('admin') ) // Массовая редактирование планироваок
 {
		$query = 'UPDATE `apartaments` SET `image_pb` = "'.$_POST[newplan].'" WHERE `home_id` = "'.$_GET[home].'"  AND  ( `apartment_num` = "100000"   ';
		foreach($_POST[editapart][$_GET[home]] as $k=>$v )
		{
			print 'Новая планировка для ' . unit_label('gen') . ' '.$k.' : '.$_POST[newplan].' <br/>';
			// формируем запрос на обновление цены по ид дома и хаты
			$query.= ' OR `apartment_num` = "'.$k.'" ';
		}
		$query.= ');';
...
if(trim($_POST['newplan_floor']) &&  check_access('admin') ) // Массовая редактирование планироваок
 {
  ...
  if(trim($_POST[newarea]) &&  check_access('admin') ) // Массовая редактирование планироваок
 {
		$query = 'UPDATE `apartaments` SET `area` = "'.$_POST[newarea].'" WHERE `home_id` = "'.$_GET[home].'"  AND  ( `apartment_num` = "100000"   ';
		foreach($_POST[editapart][$_GET[home]] as $k=>$v )
		{
			print 'Новая площадь  для ' . unit_label('gen') . ' '.$k.' : '.$_POST[newarea].' <br/>';
			// формируем запрос на обновление цены по ид дома и хаты
			$query.= ' OR `apartment_num` = "'.$k.'" ';
		}
		$query.= ');';
 ...
 if(trim($_POST[newrooms]) &&  check_access('admin') ) // Массовая редактирование планироваок
 {
		$query = 'UPDATE `apartaments` SET `rooms` = "'.$_POST[newrooms].'" WHERE `home_id` = "'.$_GET[home].'"  AND  ( `apartment_num` = "100000"   ';
		foreach($_POST[editapart][$_GET[home]] as $k=>$v )
		{
			print 'Новое количество комнат для ' . unit_label('gen') . ' '.$k.' : '.$_POST[newrooms].' <br/>';
			// формируем запрос на обновление цены по ид дома и хаты
			$query.= ' OR `apartment_num` = "'.$k.'" ';
		}
		$query.= ');';
  ...
  if(trim($_POST[newtext]) &&  check_access('admin') ) // Массовая редактирование планироваок
 {
		$query = 'UPDATE `apartaments` SET `text` = "'.$_POST[newtext].'" WHERE `home_id` = "'.$_GET[home].'"  AND  ( `apartment_num` = "100000"   ';
		foreach($_POST[editapart][$_GET[home]] as $k=>$v )
		{
			print 'Новое примечание для ' . unit_label('gen') . ' '.$k.' : '.$_POST[newtext].' <br/>';
			// формируем запрос на обновление цены по ид дома и хаты
			$query.= ' OR `apartment_num` = "'.$k.'" ';
		}
		$query.= ');';
...
if(trim($_POST[newstatus]) &&  check_access('admin') ) // Массовая редактирование Статуса
 {
