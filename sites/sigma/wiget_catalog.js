   var m2WidgetConfig = (function () {
       ...
       return {
           ...
           ajaxRouter: cfg.ajaxRouter || (current && current.dataset.ajaxRouter) || (String(sahmatkaUrl).replace(/\/+$/, '') + '/ajax_router.php'),
           ajaxActions: cfg.ajaxActions || (current && current.dataset.ajaxActions) || (String(sahmatkaUrl).replace(/\/+$/, '') + '/ajax_actions.php'),
           jqueryUrl: cfg.jqueryUrl || (current && current.dataset.jqueryUrl) || (String(sahmatkaUrl).replace(/\/+$/, '') + '/template/default/libs/jquery-3.3.1/jquery-3.3.1.min.js')
       };
   })();

   if (!window.$) {
       var script = document.createElement('script');
       script.src = m2WidgetConfig.jqueryUrl;
   ...
   $(document).ready(function() {
       ...
       */
	   ...
	   function loaddata()
	   {
		   ...
		   sendAjaxForm('zapisdata', 'filtrform', m2WidgetConfig.ajaxRouter + '?ctr=apartments&act=data', 0, 'progressbar', preload , postload ); 

		  
		   //  sendAjaxForm('zapisdata', 'filtrform', 'https://xdemo.m2profi.pro/sahmatka/ajax_actions.php?load=data&controller=catalog', 0, 'progressbar', preload , postload ); 
	   }
               ...
               array.forEach(function(item, i, arr) {
                   if (item) {
                       ...
                       var preload = function(item) {
                           ...
                           //  sendAjaxForm(item, formid, 'https://xdemo.m2profi.pro/sahmatka/ajax_actions.php?load=' + item + '&controller=' + controller, 1, 'progressbar', preload()); // Грузим содержимое селек
					   
					     sendAjaxForm(item, formid, m2WidgetConfig.ajaxRouter + '?ctr=apartments&act=' + item, 1, 'progressbar', preload()); // Грузим содержимое селек
