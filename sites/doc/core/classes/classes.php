...
$conf = $config[$home][$section];
 ...
 $endnum = ($conf['floor']*$conf['apartments']) + $conf['start_num']-$ckv; // Количество квартир в секции
	...
	for($i=$conf['floor']; $i>0; $i--)
	{
		...
		if( ($_SESSION['sh_login'] == 'admin' || $_SESSION['sh_login'] == 'demo_admin') && $i==$conf['floor'])
		{
			?>
				<thead>
					  <tr>
			  ...
			  <option value="6">Квартира подрядчика</option>
				*/
				elseif($data[$i][$k][status]=='0' || $data[$i][$k][status]=='1' || !$data[$i][$k] ) // Нет информации
				{
					...
					if($_SESSION['adm_caption'] && $_SESSION['sh_login']!='admin' &&  $_SESSION['sh_login'] != 'demo_admin') //  админ агентства
					{
						$class="tdgreen";
						$value='<a class="iframe_r" href="#1'.$_SESSION['sh_login'].'" style="font-size:14px; color:#FFF;" ><b>№ '.$end_etza_num.'</b></a>';
					}
					elseif($_SESSION['sh_login'] == 'admin'  || $_SESSION['sh_login'] == 'demo_admin')
					{
						$class="tdgreen";
				$value='
				...
				if($conf['clean_apartments'][$i][$k]) // пустая квартира
				{
					...
					*/
					...
					?>
					<td  <? if($_SESSION['sh_login'] != 'admin' &&  $_SESSION['sh_login'] != 'demo_admin'){print 'style="height:auto;"';} else{print 'style="height:74px;"';}?>>
 ...
 if(!$notable)
 {
			...
			*/
			...
			else // ИНфа о квартире
			{
				...
				elseif($apart[status]=='5')
				{
					...
					if( $_SESSION['sh_login'] == 'admin'  || $_SESSION['sh_login'] == 'em_nsv' || $_SESSION['sh_login'] == 'director' || $_SESSION['sh_login'] == 'demo_admin')
					{
						$class="tdgrey";
	...
	// (разные шаблоны вывода квартир)
				if($conf['clean_apartments'][$i][$k]) // пустая квартира
				{
					...
					{
						?>
						<td class="<?=$class?>" rel="tooltip"  title="<a style='color:#FFF;' href='<?=$edit_url?>'><?=$stext?><br/><?=$status_text?></a>" <? if($_SESSION['sh_login'] != 'admin'){print 'style="height:auto;"';}?>>
						<?=$value?>
						</td>
						<?
					}
					...
					{
						?>
						<td  <? if($_SESSION['sh_login'] != 'admin' && $_SESSION['sh_login'] != 'demo_admin'){print 'style="height:auto;"';} else{print 'style="height:74px;"';}?>>	<?=$value?> </td>			
						<?
					}
				}
				else
...
$ckv=0;
 ...
 $endnum = ($conf['floor']*$conf['apartments']) + $conf['start_num']-$ckv; // Количество квартир в секции
			...
			*/
			...
			else // ИНфа о квартире
			{
				...
				else // Все остальные проданы
				{
					...
					$edit_url = 'https://' . $GLOBALS['config']['domain'] . '/'sahmatka/form_order.php?home_id='.$home.'&apartment_num='.$end_etza_num.'&apartments='.$k;
					$end_etza_num2 = '№'.$end_etza_num;
	...
	// (разные шаблоны вывода квартир)
				...
				{
					?>
					<td  <? if($_SESSION['sh_login'] != 'admin' && $_SESSION['sh_login'] != 'demo_admin'){print 'style="height:auto;"';} else{print 'style="height:74px;"';}?>></td>
					<?
				}
				else
				{
			...
			*/
			...
			else // ИНфа о квартире
			{
				...
				else // Все остальные проданы
				{
					...
					if($_GET['new']==1)
					{
					$edit_url = 'https://lg.m2profi.pro/sahmatka/iframe_router.php?ctr=apartments&act=card&home_id='.$home.'&apartment_num='.$end_etza_num.'&apartments='.$k;
					}
					elseif($_GET['new']==2)
					{
							$edit_url = 'https://' . $GLOBALS['config']['domain'] . '/'sahmatka/iframe_router.php?ctr=apartments&act=card&home_id='.$home.'&apartment_num='.$end_etza_num.'&apartments='.$k;
					}
 ...
 $endnum = ($conf['floor']*$conf['apartments']) + $conf['start_num']-$ckv; // Количество квартир в секции
	...
	// (разные шаблоны вывода квартир)
				...
				else
				{
					...
					<?=$status_text?>" 
					<? if($_SESSION['sh_login'] != 'admin'){print 'style="height:auto; '.$st_bg.'"';}else{print 'style=" '.$st_bg.' " ';}?>
					>
						<?=$value?>
					</td>
					<?
