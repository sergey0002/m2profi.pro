<?
		...
		//	include('new2/messages/PHPMailer/PHPMailer.php');
		//	include('new2/messages/PHPMailer/SMTP.php');
...
class SMTP
{
    ...
    public function getLastTransactionID()
    {
        return $this->last_smtp_transaction_id;
			 ...
			 class messages
			 { 
				 function __construct($host='ssl://smtp.mail.ru',$login='energomontaz452@mail.ru',$password='Fkt67kd7h9CpgmKhTkcq',$name=$GLOBALS['config']['domain'] )
				 {
					global $mysql;
					$this->mysql = $mysql;
					...
					$this->login = 'energomontaz452@mail.ru';                     
					//$this->password = 'vdSnRIR43yo]'; 
					$this->password ='Fkt67kd7h9CpgmKhTkcq'; // пароль приложения
		 
				
					$this->from_name = $GLOBALS['config']['domain'];		
					...
					$this->host = $host;	 
					$this->login = $login;                     
					$this->password = $password; // пароль приложения
					$this->from_name = $name;		
					
					
 
					$this->from_email = $this->login;

					// Параметры подключения mysql
					//$this->sql_host = 'localhost';	 
					//$this->sql_login = 'm2profi_demo';                     
					//$this->sql_password = 'WOU7GRi8Ss'; 
					//$this->sql_base = 'm2profi_demo'; 
					// $this->sql_table = 'messages';
					
					
					//$this->sql_login = $GLOBALS['config']['mysql_login'];
					//$this->sql_password = $GLOBALS['config']['mysql_password'];
					//$this->sql_base =  $GLOBALS['config']['mysql_base'] = 'm2profi_em';
					   
					//$this->connection = mysqli_connect($this->sql_host, $this->sql_login, $this->sql_password, $this->sql_base) or die(mysqli_error()); // Соединение с базой данных 
					...
					try 
					{
						...
						$this->mail=$mail; // КЛасс php mailer готовый к отправки писем
					} 
					catch (Exception $e) 
					{
						print '<script>alert("'.$mail->ErrorInfo.'");</script>';
				 ...
				 function insert_sql_record($to='' , $subject='' , $text='' , $type='' ,$params_array = '')
				 {
					 
					  $to = str_replace('89236470002@mail.ru,','',$to);
					  $to = str_replace('89236470002@mail.ru','',$to);
					  ...
					  $data['date']="NOW()";
					  // print_r($data);
					  $mess_id = $this->mysql-> insert( 'messages', $data);
					  
					if(is_array($params_array) && $mess_id)
					{
						// print_r($params_array);
						foreach($params_array as $k=>$v)
						{
							$data = array();
							$data['messages_id']=$mess_id;
							$data['filed_name']=$k;
							$data['value']=$v;
						
							$mess_p_id = $this->mysql-> insert( 'messages_fileds', $data);
						}
					}
				 }
				...
				{ 
					...
					<table>
						...
						<tr>
							...
							<th><a href="#"><b>Получатель</b></a></th>
							<th><a href="#"><b>Тема</b></a></th>
							<th><a href="#"><b>Текст</b></a></th>
						</tr>
					</thead>
					<tbody>
					...
					$q = 'SELECT * FROM `messages` ORDER BY `messages_id` DESC '; 
					$arr = $this->mysql->get_arr($q);
					foreach($arr as $k=>$result)
					{	
						print '
						<tr>
						<td>'.$result['date'].'</td>
				 ...
				 // Отправка сообщения
				 function send( $to , $subject , $text , $type = 'Форма на главной', $alert='Спасибо за обращение, специалист свяжется с вами в ближайшее время',$mess_params='' )
				 {  
						
						$this->insert_sql_record($to , $subject , $text , $type ,$mess_params); // Фиксируем в MY SQL
						  
						$buf = explode(',',$to);  
						foreach($buf as $k=>$v)
						{
							$v=trim($v);
							$this->mail->addAddress($v);     // Add a recipient
						}
						...
						{
							print '<script>alert("Ошибка");</script>';
						} 
		 	...
		 	// $mess44 = new messages('ssl://smtp.mail.ru','energomontaz452@mail.ru','Jm9PqZBYjykch7ThSZKt');
       
			// $mess44 ->send( '89236470002@mail.ru ,energomontaz452@mail.ru' ,'Т11111111Ема' , 'ТЕкст' );
				 
			// $mess44 ->display_messages();
