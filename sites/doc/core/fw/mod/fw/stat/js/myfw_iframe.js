...
{
	if(!url){ url = $('#'+formid).attr('data-ajaxurl');	}	if(!url){alert('error'); return;}
...
{ 
	...
	array.forEach(function(item, i, arr) 
	{
		...
		sendAjaxForm( item , formid , '/ajax_router.php?ctr='+controller+'&act='+item , 1 , 'progressbar', predcallback , predcallback2 , postcallback ); // Грузим содержимое селек
...
sendAjaxForm( 'ид куда грузить' , 'ид формы' , 'https://em.m2profi.pro.test/sahmatka/ajax_actions.php?load=data&controller=zapiskeys',0); // Грузим содержимое селек
			...
			success: function(data){
				...
				//	$('#'+result_src_id).fadeIn(3000);
					...
					if(!msg['error'])
					{
						...
						iprew = ' <div class="fiprew ui-sortable-handle"><div class="fiprew_links fiprew_links_main" style="display:none;"> 	<a href="" title="Главная">i</a> </div>	<div class="fiprew_links fiprew_links_del" > <a href="" title="Удалить">x</a> </div>	<img src="'+msg['icon']+'" /><input type="hidden" name="'+iprew_name+'['+lastfnum+']'+'"  value="'+msg['link']+'" /><br/>'+msg['file']+' </div>';
...
$(".fiprew_links_del").click(function()
{
	...
	$.ajax({
		url: '/ajax_router.php?ctr=nodes&act=ajax_delimg',
		method: 'post',
		dataType: 'html',
