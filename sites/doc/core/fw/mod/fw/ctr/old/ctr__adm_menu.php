...
+ При изменении элемента тоже присваивать ему черную границу пр доавлении!

+ Сохранение форм в сессиях! где прописать занчение селектов и текстовых форм + кнопка сброса фильтров
+ перевод на json - количество найденных элементов? возможность перевода

+ ФОрмы в раскрывашках?!
...
------------------
Таблицы унитабле !
поля непосредственно в выводе!

задаем только тип сущности и поля которые хотим 
+ настраиваем редактор (форму редактирвоания!)
...
+ прогрессбар в контент часть не вешать вешать после или глобальный прогрессбар 
 
 
 
 
*/
class ctr__adm_menu extends ctr__
{ 

	var $table = 'menu'; //Главная таблица
	var $key_filed = 'menu_id'; // Ключевое поле главной таблицы
	var $ctr = 'adm_menu';
	...
	{
		global $mysql;
		$q = 'SELECT menu.*, dir.dir_id, dir.dir_title';

		//if( $_REQUEST['dir'] ) { $q.= ', count(dir) as c ';}

        $q.=' FROM menu 
		LEFT JOIN dir ON dir.dir_id = menu.dir
		WHERE  menu.del != "2" ';
		
		// Фильтрация раздела
		if( $_REQUEST['dir']){ $q.=' AND dir.dir_id = "'. $_REQUEST['dir'].'" ';}
		if( $_REQUEST['id']){ $q.=' AND menu.menu_id = "'. $_REQUEST['id'].'" ';}
		
		// Удаленные
		if( !$_REQUEST['show_dell']){ $q.=' AND menu.del = "0" '; }
		else{ }
		...
		{
			$search_arr = array();
			$search_arr[]='menu.title';
			$search_arr[]='menu.descr';
		...
		<section class="section-stat">
			<div class="container mobc"  >
				<div class="page-header">
					...
					<div class="page-header__title">Меню</span></div>
				</div>
