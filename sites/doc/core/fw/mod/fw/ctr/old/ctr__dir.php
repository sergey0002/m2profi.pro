		...
		$(function () {
			...
			$('#tree')
				.jstree({
					'core' : {
						'data' : {
							'url' : '/admin/ajax_router.php?ctr=dir&act=get_node_jsoon',
							'data' : function (node) {
								return { 'id' : node.id };
							}
						},
				...
				.on('delete_node.jstree', function (e, data) {
					$.get('/admin/ajax_router.php?ctr=dir&act=delete_node_jsoon', { 'id' : data.node.id })
						.fail(function () {
							data.instance.refresh();
						});
				})
				.on('create_node.jstree', function (e, data) {
					$.get('/admin/ajax_router.php?ctr=dir&act=create_node_jsoon', { 'id' : data.node.parent, 'position' : data.position, 'text' : data.node.text })
						.done(function (d) {
				...
				.on('rename_node.jstree', function (e, data) {
					$.get('/admin/ajax_router.php?ctr=dir&act=rename_node_jsoon', { 'id' : data.node.id, 'text' : data.text })
						.fail(function () {
							data.instance.refresh();
						});
				})
				.on('move_node.jstree', function (e, data) {
					$.get('/admin/ajax_router.php?ctr=dir&act=move_node_jsoon', { 'id' : data.node.id, 'parent' : data.parent, 'position' : data.position })
						.fail(function () {
							data.instance.refresh();
						});
				})
				.on('copy_node.jstree', function (e, data) {
					$.get('/admin/ajax_router.php?ctr=dir&act=copy_node_jsoon', { 'id' : data.original.id, 'parent' : data.parent, 'position' : data.position })
						.always(function () {
							data.instance.refresh();
						});
				})
				.on('changed.jstree', function (e, data) {
					if(data && data.selected && data.selected.length) {
						...
						$.get('/admin/ajax_router.php?ctr=dir&act=get_content_node_jsoon', { 'id' : data.node.id})
						.done(function (d) {	 
							// data.instance.set_id(data.node, d.id);
