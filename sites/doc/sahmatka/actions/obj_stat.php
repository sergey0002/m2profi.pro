...
$sql="select 
  ...
  COUNT(
    IF(broni.status = null, 1, NULL)
  ) as cn, 
  COUNT(
    IF(broni.status = 1, 1, NULL)
  ) as c1, 
  COUNT(
    IF(broni.status = 2, 1, NULL)
  ) as c2, 
  COUNT(
    IF(broni.status = 3, 1, NULL)
  ) as c3, 
  COUNT(
    IF(broni.status = 4, 1, NULL)
  ) as c4, 
  COUNT(
    IF(broni.status = 5, 1, NULL)
  ) as c5, 
  COUNT(
    IF(broni.status = 6, 1, NULL)
  ) as c6 ,
...
from 
  ...
  LEFT JOIN broni ON (
    apartaments.home_id = broni.home_id 
    AND apartaments.apartment_num = broni.apartments_num 
    AND broni.date =(
      select 
        max(date) 
      from 
...
$new_arr[2]['all'] = $new_arr['c']['all']-$new_arr[3]['all']-$new_arr[4]['all']-$new_arr[5]['all']-$new_arr[6]['all']-300;
