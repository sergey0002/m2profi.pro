...
{  
	...
	function printspace_disp($data='')
	{
		...
		{
			$href   = $GLOBALS['config']['base_url'].'/sahmatka/iframe_router.php?ctr=parking_spaces&act=order&id='.$data['parking_space_id'].'';
		...
		{
			if($data['status']!=2){$price='';}else{$price.='<br/>';}
			$href   = $GLOBALS['config']['base_url'].'/sahmatka/iframe_router.php?ctr=parking_spaces&act=broni&id='.$data['parking_space_id'].'';
			?>
