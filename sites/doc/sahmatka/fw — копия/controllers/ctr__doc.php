 ...
 function act__index()
 {
 	...
 	function act__get_tree_data()
	{
		...
		foreach ($dirs as $dir) {
			...
			$tree_data[] = [
				...
				'data' => [
					'deleted' => $is_deleted,
                    'order' => (int)$dir['order'],
                    'sort_date' => 0 // Папки не имеют даты документа, но можно использовать id для вторичной сортировки
				]
			];
		}
        ...
        usort($tree_data, function($a, $b) {
            ...
            // Если order одинаковый (например 0), сортируем по дате (для файлов) или ID (для папок)
            // Для простоты используем sort_date для файлов, а для папок можно взять ID (как прокси времени создания)
            // Но чтобы папки и файлы перемешивались корректно при равном order, нужна единая логика.
            // Пока оставим вторичную сортировку по ID (новые выше)
            
             // Извлекаем числовой ID для вторичной сортировки
    ...
    function act__move_node()
    {
        ...
        // Важно: исключаем удаленные, если они не отображаются, но для целостности порядка лучше брать все активные (del=0)
        // Если пользователь видит удаленные, то порядок может сбиться при скрытии.
        // Будем сортировать только активные (del=0), так как удаленные обычно не участвуют в drag-n-drop
        ...
        // Объединяем и сортируем текущий список (без перемещаемого узла, если он уже там был, но мы его обновили выше, так что он там есть)
        // Но нам нужно сформировать список БЕЗ перемещаемого узла, чтобы вставить его в нужную позицию.
