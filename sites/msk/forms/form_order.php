...
<script src="libs_n/jquery-3.3.1/jquery-3.3.1.min.js"></script>
	<script>
...
if($_GET['action']  && $_POST[name] && $_POST[phone])
 ...
 //print '</pre>';
		...
		$message .= "Дом <b>".$homes[$home_id]['caption']."</b> секция-<b>".$_GET[section_id]."</b> Этаж-<b>". $_GET[floor]."</b> кв-<b>".$apartment_num."</b> ";
 
			 
	 include('messages/class.php');
     $mess = new messages();
	
	 $mess->send( '89236470002@mail.ru,op@em-nsk.group' ,'Заявка с сайта EM-NSK.ru - ФОРМА КАРТОЧКИ КВАРТИРЫ -'.$act.' '.$_POST['name'].' ' , $message );
 	// $mess->send( 'op@em-nsk.group' ,'Заявка с сайта EM-NSK.ru - ФОРМА КАРТОЧКИ КВАРТИРЫ -'.$act.' '.$_POST['name'].' ', $message );
...
if(!$stat || $stat==2)
{
	?>
				 ...
				 <div style="margin:10px; margin-left:0;" class="noprint">
					 ...
					 <a href="#" onClick="window.print();"><img src="static/img/p.png" width="40px" style="margin:5px; margin-left:0;"></a>
					 <a onclick="$('#one').slideToggle('slow');" href="javascript://"><img src="static/img/m.png" width="40px" style="margin:5px;"></a>
	...
	<ul class="tabs__caption" style="padding-left:0;">
		...
		<li>Забронировать</li>
		<a href="exc.php" target="_top"><li >Записаться на экскурсию</li></a>
	</ul>
